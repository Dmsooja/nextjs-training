import React from 'react';
import { RichText } from 'prismic-reactjs';
import { Link } from 'prismic-reactjs';

const FeaturedProducts = ({ slice }) => (
  <section>
    <div className="bg-white">
      <div className="max-w-2xl mx-auto py-16 px-4 sm:py-24 sm:px-6 lg:max-w-7xl lg:px-8">
        <div className="text-2xl font-extrabold tracking-tight text-gray-900"><RichText render={slice.primary.title} /></div>

        <div className="mt-6 grid grid-cols-1 gap-y-10 gap-x-6 sm:grid-cols-2 lg:grid-cols-4 xl:gap-x-8">
          {slice?.items?.map((item, idx) => (
            <div key={idx} className="group relative">
              <div className="w-full min-h-80 bg-gray-200 aspect-w-1 aspect-h-1 rounded-md overflow-hidden group-hover:opacity-75 lg:h-80 lg:aspect-none">
                <img
                  src={item.image.url} alt={item.image.alt}
                  className="w-full h-full object-center object-cover lg:w-full lg:h-full"
                />
              </div>
              <div className="mt-4 flex justify-between">
                <div>
                  <div className="text-sm text-gray-700">
                    <a href={Link.url(item.link)}>
                      <span aria-hidden="true" className="absolute inset-0" />
                      <RichText render={item.name} />
                    </a>
                  </div>
                  <div className="mt-1 text-sm text-gray-500"><RichText render={item.variation} /></div>
                </div>
                <div className="text-sm font-medium text-gray-900"><RichText render={item.price} /></div>
              </div>
            </div>
          ))}
        </div>
      </div>
    </div>
  </section>
)

export default FeaturedProducts